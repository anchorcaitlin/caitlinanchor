% This program provides a simulation of the trajectory of a non-horizontally 
% launched projectile in the absence of air resistance. The program outputs 
% computations of the projectile's range, maximum height, and time of flight 
% of the projectile based upon known parameters inputted by the user. 

clc; % Clear the command window. 
clear; % Clear all current variables in the workspace.
...
elseif ismember([4, 5, 6], indexList) % If all of the elements [4, 5, 6] 
    ...
    % If the user has selected a different combination of options, there is
    % no method to calculate the time of flight, range and maximum height
    % of the projectile.
...
%  ------------------- PLOT ANIMATED GRAPH OF PROJECTILE -----------------
...
if (isequal(indexList, [1, 2, 3]) == 1) || (isequal(indexList, [1, 3, 5]) == 1) ...
    ...
    for i = 1 : length(timeXAxis)
        subplot(3, 1, 1);
        plot(timeXAxis(i), verticalHeightYAxis(i), '-or');
        title 'Vertical displacement of projectile over time'
        xlabel('Time (s)');
        ylabel('Vertical height (m)');
        hold on; 
        grid on;
        % Set the dimensions of the x and y axis. 
        axis([min(timeXAxis), max(timeXAxis), min(verticalHeightYAxis)-1, ...
            max(verticalHeightYAxis)+1]);
        pause(0.03); % Creates animation of plot, controls the speed.
    end
    ...
    for i = 1 : length(timeXAxis)
        subplot(3, 1, 2);
        plot(timeXAxis(i), velocity(i), '-xb');
        ...
        axis([min(timeXAxis), max(timeXAxis), min(velocity)-1, ...
            max(velocity)+1]); 
        pause(0.03); % Creates animation of plot. 
    end
    ...
    for i = 1 : length(timeXAxis)
        subplot(3, 1, 3);
        plot(timeXAxis(i), acceleration, '-ok');
    ...
    % Print results of projectile motion calculations to user. 
    messageCalculations = sprintf(" The projectile reaches the ground at " + ...
        "%.2fs\n The projectile has a maximum height of %.2fm\n " + ...
        "The range of the projectile is %.2fm\n", time, maximumHeight, rangeProjectile);
    message = sprintf('%s', messageCalculations);
    uiwait(helpdlg(messageCalculations, 'Projectile Motion Calculations'));
end 